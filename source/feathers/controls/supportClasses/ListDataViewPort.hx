...
class ListDataViewPort extends FeathersControl implements IViewPort
{
	...
	private function refreshRenderers():Void
	{
		...
		this.freeInactiveRenderers(this._minimumItemCount);
	...
	private function freeInactiveRenderers(minimumItemCount:Int):Void
	{
		//we may keep around some extra renderers to avoid too much
		//allocation and garbage collection. they'll be hidden.
		var itemCount:Int = this._inactiveRenderers.length;
		var keepCount:Int = minimumItemCount - this._activeRenderers.length;
		trace(this._inactiveRenderers.length +' ' + this._activeRenderers.length+' ' + minimumItemCount);
		...
		//fixbug: haxe bug itemCount diif this._inactiveRenderers.length;
		if(itemCount<this._inactiveRenderers.length)
		{
			itemCount = this._inactiveRenderers.length;
		}
