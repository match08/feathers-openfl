...
class ApplicationMain
{
	...
	{
		lime.system.System.__registerEntryPoint("Video", create);

		#if (js && html5)
		#if (munit || utest)
		lime.system.System.embed("Video", null, 960, 640);
		#end
		#else
		create(null);
		#end
	...
	macro public static function getEntryPoint()
	{
		...
		switch (Context.follow(Context.getType("Video")))
		{
			case TInst(t, params):
				...
				if (hasMain)
				{
					return Context.parse("@:privateAccess Video.main()", Context.currentPos());
				}
				...
				{
					Context.fatalError("Main class \"Video\" has neither a static main nor a constructor.", Context.currentPos());
				}

			default:

				Context.fatalError("Main class \"Video\" isn't a class.", Context.currentPos());
		}
...
@:keep @:dox(hide) class DocumentClass extends Video {}
#else
class DocumentClass
{
